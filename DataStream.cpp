...
void DataStream:: rcpLoop()
{
  ...
  const unsigned long requestInterval = 6000; // 10sekund
  const unsigned long refreshInterval = 6000; // 20minut
  ...
  while(i == 0 )
  {
    ...
    // if((currentTime - lastRequestTime >= requestInterval)) /// Petla dla poboru danych
    // {
...
void DataStream:: ledDriver()
{
  ...
  if (employeeEvent == "1" )   ///przerwa 1 = poczatek przerwy 
  {
  ...
  else if (employeeEvent == "2") //|| (employeeEvent = 3) || (employeeEvent = 4)  )  /// praca        2 = koniec przerwy , 3 = roczpoczecie pracy 4 = powrot z wyjscia sluzbowego
  {
    digitalWrite(12,LOW);
  ...
  else if (employeeEvent == "3" )  ///poza praca 5 = koniec pracy
  {
    digitalWrite(12,LOW);
    digitalWrite(13,LOW);
  ...
  else if (employeeEvent == "4" ) ///wyjscie sluzbowe 6 = wyjscie sluzbowe
  {
    digitalWrite(12,LOW);
    digitalWrite(13,LOW);
    digitalWrite(14,LOW);
...
{
 // Dekodowanie znaków Unicode \uXXXX, jeśli istnieją
    response.replace("\\u003C", "<");
    response.replace("\\u003E", ">");
    response.replace("\\u0022", "\"");
    ...
    // Można dodać więcej zamian w razie potrzeby

    const String start_marker = "<span class=\"fw-bolder fs-2\">";
    // POPRAWKA: Szukamy znacznika z zabezpieczonym ukośnikiem
    ...
    if (end_pos == -1) 
    {
      // Zwracamy fragment odpowiedzi do debugowania, żeby zobaczyć, co poszło nie tak
      return "Blad: Nie znaleziono konca statusu w: " + response.substring(start_pos, start_pos + 50);
    }
